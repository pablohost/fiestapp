...
if(isset($_POST["x"])&&isset($_POST["y"])&&isset($_POST["z"])){
	...
	$sql = 'SELECT Usuarios.nombre,Usuarios.apelli,Usuarios.indTip,Eventos.ind,Eventos.fecIni,Eventos.fecFin,Eventos.titulo,Eventos.fly,Eventos.estado,Locales.lon,Locales.lat,Locales.nombre
        ...
        AND Eventos.estado BETWEEN 0 AND 1;';
									...
									<div class="container-fluid pt-2 boxDes">
										<div class="row">
											...
											<a href="evento?titulo='.$tituloEvento.'&x='.$indiceEvento.'" style="color: black;">
												<p class="infoProd">'.strtoupper($tituloEvento).'</p>
											</a>
											</div>
											<div class="col-6">
												<p class="text-muted mb-1"><i class="fas fa-map-marker-alt"></i> '.$nombreLocal.'</p>
											</div>
											<div class="col-6">
												<p class="text-muted"><i class="fas fa-calendar-day"></i> '.ucfirst(strftime("%a, %d de %B", strtotime($fechaInicio))).'</p>
											</div>
										</div>
									</div>
								</form>
