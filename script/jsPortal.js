	...
	//boton galeria recinto - organizador
      ...
      let z=$(this).prevAll("img").attr("src");
	    Swal.fire({
		  ...
		  if (result.value) {
		      setTimeout(function(){ borraFotoRecinto(x,z); },2500)
...
function cargaEventosOrga(){
	...
	$.post('script/cargaEventosOrga.php',
      ...
      $("#medio .boxEve").each(function(){
              $(this).find("#botoneraEve").empty();
          });
		}
...
function borraFotoRecinto(indice, ruta) {
	// body...
	$.ajax({
	    type: "POST",
	    url: 'script/comprobarIdentidad.php',
	    ...
	    success: function (data) {
	      	if (data.cod==0) {
	      		$.get( "script/borraFotoRecinto.php?indice="+indice+"&ruta="+ruta, function( respuesta ) {
				  if (respuesta.cod === 0) {
			            Swal.fire({
			                title: "Listo!",
			                text: respuesta.msg,
